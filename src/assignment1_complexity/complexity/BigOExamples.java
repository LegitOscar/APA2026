package assignment1_complexity.complexity;

import java.util.ArrayList;

public class BigOExamples {

    // O(1) - array index = direct memory address, size irrelevant.
    public static int getByIndex(ArrayList<Integer> numbers, int ID) {
        return numbers.get(ID);
    }

    // O(n) - one loop, touches each element once.
    ...
    // O(log n) - binary search, halves search space each step. List must be sorted
    ...
    // O(n²) - nested loop, inner loop starts at i+1 to avoid rechecking pairs
